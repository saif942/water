package com.water.login.clockINclockOUT.util;

public class Constant {

    public static final String CHECK_IN ="IN";
    public static final String CHECK_OUT ="OUT";
    public static final String CHECKOUT_MESSAGE = "Successfully checked out";
    public static final String CHECK_IN_MESSAGE = "Successfully checked In";
    public static final String INVALID_USER = "Invalid user ID provided";
    public static final String USER_NOT_IN_CHECK_IN_OUT="User not found";
    public static final String USER_ALREADY_IN="User is already checked IN";
    public static final String USER_ALREADY_OUT="User is already checked OUT";
    //table name
    public static final String USER_TABLE="Water_Employs";
    public static final String IN_OUT_LOG_TABLE = "Check_in_out_log" ;
    public static final String IN_OUT_TABLE = "Check_in_out";

}
